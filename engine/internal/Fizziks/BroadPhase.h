...
public:
	...
	virtual uint32_t add(uint32_t ID, const AABB& aabb) = 0;
	virtual bool remove(uint32_t ID) = 0;
	virtual void replace(uint32_t prevID, uint32_t newID) = 0;
	virtual void update(uint32_t ID, const AABB& aabb) = 0;
	...
	virtual std::vector<uint32_t> query(const AABB& aabb) const = 0;

	virtual RaycastResult raycast(const Ray& ray) const = 0;

	virtual std::vector<AABB> getDebugInfo() const = 0;
};
}
